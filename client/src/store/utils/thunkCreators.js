...
export const postMessage = (body) => async (dispatch) => {
  try {
    ...
    if (!body.conversationId) {
      dispatch(addConversation(body.recipientId, newData.message));
    } else {
      dispatch(setNewMessage(newData.message));
    }

    sendMessage(newData, body);
  } catch (error) {
    console.error(error);
  }
};
...
export const readMessages = (convoId) => async (dispatch) => {
  try {
    ...
    dispatch(updateConvo(updatedConvo));
  } catch (error) {
...
export const mutualConvoId = (convoId) => async (dispatch) => {
  try {
    ...
    dispatch(checkMutualConvo(mutualConvo));
  } catch (error) {
    console.log(error.message);
  }
};
